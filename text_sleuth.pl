...
my ($byte_length, $pattern_file, $search_path, $wildcard, $ignore);
	...
	for(my $i = 0; $i <= $size - $pattern_span; $i ++)
	{
		...
		if($valid)
		{
			...
			if($first_match)
			{
				...
				# Otherwise, retain original path formatting.
				else
				{
					# Status message.
					print "> " . $file;
			...
			foreach my $group (@formatted_groups)
			{
				...
				# If 16 groups have been printed and more remain, insert a newline and
				# indent the next line to maintain alignment.
				if($group_count % 16 == 0 && $group_count < scalar(@formatted_groups))
				{
					print "\n    ";
				}
			}

			# Ensure the output ends with a newline, unless the final group already
			# ended on a clean 16-group boundary.
